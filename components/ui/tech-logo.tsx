  ...
  const { techData, logoUrl } = React.useMemo(() => {
    ...
    if (customLogoUrl) {
      ...
      if (customLogoUrl.startsWith('/icons/') && techData) {
        ...
        const filename = pathParts[pathParts.length - 1]; // e.g., "nextjs-original.svg"
        const versionPart = filename.split('-')[1]; // e.g., "original.svg"
        const versionName = versionPart?.replace('.svg', '') || 'original'; // e.g., "original"
